...
async function sendApprovalEmail(order: any, toCompany: any, fromCompany: any, items: any[]) {
  ...
  const fromName = fromCompany?.name || ''
  const html = '<div style="font-family:sans-serif;max-width:600px;margin:0 auto;">'
    + '<h2 style="color:#1d4ed8;border-bottom:2px solid #1d4ed8;padding-bottom:8px;">&#128230; ' + order.order_no + ' - ' + fromName + ' からの発注承認依頼</h2>'
    ...
    + '<tr><td style="padding:6px 0;color:#666;">発注元</td><td>' + fromName + '</td></tr>'
    ...
    + '<a href="' + approveUrl + '" style="display:inline-block;background:#16a34a;color:white;padding:14px 32px;border-radius:6px;text-decoration:none;font-weight:bold;font-size:16px;margin:16px 0;">&#9989; ワンクリックで承認する</a>'
    + '<p style="color:#6b7280;font-size:12px;">このリンクは7日間有効です。Order Relay システムから自動送信されました。</p></div>'
  ...
  await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      to: [toCompany.email],
      subject: '[承認依頼] ' + order.order_no + ' - ' + fromName,
      html
    })
  })
}
...
async function sendCompletionEmail(order: any, finalCompany: any, fromCompany: any, items: any[]) {
  ...
  const html = '<div style="font-family:sans-serif;max-width:600px;margin:0 auto;">'
    ...
    + '<p style="color:#6b7280;font-size:12px;">Order Relay システムから自動送信されました。</p></div>'
