...
export default function FlowsPage() {
  ...
  const save = async () => {
    ...
    const { error } = await supabase.from('flows').insert({ name, steps: validSteps, doc_sets: docSets, notes: '' })
    ...
    <div>
      ...
      {showForm && (
        <div className="bg-gray-50 border rounded-lg p-4 mb-4">
          <h2 className="font-semibold mb-3">商流登録</h2>
          <div className="mb-3">
            <label className="text-sm text-gray-600">商流名*</label>
            <input className="input-field mt-1" value={name} onChange={e => setName(e.target.value)} placeholder="例: とりもつえん→創未家→ゼロテック" />
          </div>
          <div className="space-y-2">
            <label className="text-sm text-gray-600">取引ステップ（上から順番）</label>
            {steps.map((s, i) => (
              <div key={i} className="flex gap-2 items-center">
                ...
                <select className="input-field w-28" value={s.role} onChange={e => { const ns=[...steps]; ns[i]={...ns[i],role:e.target.value}; setSteps(ns) }}>
                  <option value="buyer">発注元</option>
                  <option value="middle">中間</option>
                  <option value="seller">仕入先</option>
                </select>
      ...
      {loading ? <div className="text-center py-8 text-gray-500">読み込み中...</div> : (
        <div className="space-y-3">
                ...
                <div>
                  ...
                  <div className="text-sm text-gray-500 mt-1">
                    {(f.steps as Array<{company_id:string;role:string}>).map((s,i) => (
                      <span key={i}>{i>0?'→':''}<span className="bg-blue-100 px-1 rounded">{getCompanyName(s.company_id)}</span></span>
                    ))}
                  </div>
          ...
          {flows.length === 0 && <div className="text-center py-8 text-gray-400">商流が定義されていません</div>}
        </div>
      )}
    </div>
  )
